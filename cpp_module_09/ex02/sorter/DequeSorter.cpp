...
const int DequeSorter::JACOBSTHAL_NUMBERS[DequeSorter::SEQUENCE_LENGTH] = {
 ...
 * @brief Constructor
 */
DequeSorter::DequeSorter(int argc, char **argv) : mArgc(argc), mArgv(argv) {}
 ...
 */
void DequeSorter::divideAndSwap(int sizeOfSingleGroup)
{
 ...
 */
void DequeSorter::createMainAndSubChain(std::deque< Iterator > &mainChain, std::deque< Iterator > &subChain,
 ...
 */
std::deque< DequeSorter::Iterator >::iterator DequeSorter::findInsertionLocation(std::deque< Iterator >::iterator start,
                                                                                 std::deque< Iterator >::iterator end,
                                                                                 Iterator target)
{
    std::deque< Iterator >::iterator mid;
 ...
 */
int DequeSorter::calculateSearchLength(int loopDepth, std::deque< Iterator > &mainChain)
{
 ...
 */
void DequeSorter::mergeChains(std::deque< Iterator > &mainChain, std::deque< Iterator > &subChain)
{
    int prevStartIndex = -1;

    for (int i = 0; i < DequeSorter::SEQUENCE_LENGTH; ++i)
    {
        ...
        {
            std::deque< Iterator >::iterator start = mainChain.begin();
            std::deque< Iterator >::iterator end = start + calculateSearchLength(i, mainChain);

            std::deque< Iterator >::iterator insertionLocation = findInsertionLocation(start, end, subChain[j]);
 ...
 */
void DequeSorter::updateNumbers(std::deque< Iterator > &mainChain, int sizeOfSingleGroup)
{
    std::deque< int > tempNumbers;

    for (std::deque< Iterator >::iterator it = mainChain.begin(); it != mainChain.end(); ++it)
    {
        for (int i = 0; i < sizeOfSingleGroup; ++i)
        {
 ...
 */
void DequeSorter::fordJohnsonSort(int sizeOfSingleGroup)
{
    ...
    std::deque< Iterator > mainChain, subChain;
 ...
 */
void DequeSorter::printDeque(const std::string &label) const
{
    std::cout << label << ": ";
    if (label == "Before")
    {
 ...
 */
void DequeSorter::checkAscendingOrder() const
{
